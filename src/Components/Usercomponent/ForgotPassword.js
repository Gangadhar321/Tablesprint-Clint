...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/reset-password', body);
      toast.success(response.data);
      setTimeout(() => {
        navigate('/login');
      }, 5000);
  ...
  const handleClick = async () => {
    if (isEmail(email)) {
      try {
        const response = await axios.get(`http://localhost:5000/api/forgot-password?email=${email}`);
        console.log(response);
        setSuccess(true);
        setTimeout(() => {
          setSuccess(false);
    ...
    <div className="forgot-password">
      ...
      {token ? (
        ...
        <div>
          ...
          /><br />
          <Button onClick={handleClick}>Send Email</Button><br /><br />
          <span>Go to </span><Link to="/login">Login</Link>
          {success && <p style={{ color: 'green' }}>Password reset link has been sent to your email <a href="https://mail.google.com">Go to Gmail</a></p>}
        </div>
      )}
      <ToastContainer />
    </div>
